...
export const BidderAuth = ({ setShowAuth }: BidderAuthProps) => {
    ...
    const GoogleSignInData = useCallback(async (profile: any) => {
        const { email, id } = profile;
    ...
    }, [dispatch, router, setShowAuth]);

    const GoogleSignUpData = useCallback(async (profile: any) => {
        const { email, name, id } = profile;
        try {
            console.log('reached sign up');
