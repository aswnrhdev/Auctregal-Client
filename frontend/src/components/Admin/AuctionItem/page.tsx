...
const AuctionItem = () => {
        ...
        try {
            const results = await Promise.all(
                categories.map(async (category) => {
                    const response = await instance.get(`/admin/items/${encodeURIComponent(category)}`);
    ...
    const fetchUpcomingItems = useCallback(async () => {
        try {
            const response = await instance.get('/admin/items/status/upcoming');
    ...
    const fetchBiddingItems = useCallback(async () => {
        try {
            const response = await instance.get('/admin/items/status/bidding');
    ...
    const handleSetUpcoming = async (itemId: string) => {
        try {
            await instance.put(`/admin/item/${itemId}/set-upcoming`);
            fetchItems();
            fetchUpcomingItems();
        } catch (error) {
    ...
    const handleAddToBid = async (itemId: string, duration: number) => {
        try {
            await instance.put(`/admin/item/${itemId}/add-to-bid`, { duration });
            fetchUpcomingItems();
            fetchBiddingItems();
        } catch (error) {
    ...
    const handleRemoveItem = async (itemId: string) => {
        try {
            await instance.delete(`/admin/item/${itemId}`);
            fetchItems();
            fetchUpcomingItems();
            fetchBiddingItems();
        } catch (error) {
    ...
    const closeBids = useCallback(async () => {
        try {
            await instance.post('/admin/close-bids');
            fetchBiddingItems();
        } catch (error) {
    ...
    useEffect(() => {
        ...
        const intervalId = setInterval(closeBids, 60000);
